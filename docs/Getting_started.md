...
~ The basic property of a symmetric tensor $A$ of rank $r$ is that for any permutation $σ \in S_r$,
  ...
  In this implementation, we typically select the representative index by grouping equal indices, sorting groups first in decreasing multiplicity, and then in increasing index value.
...
#from statGLOW.stats.symtensor.symtensor.permcls_symtensor import PermClsSymmetricTensor
